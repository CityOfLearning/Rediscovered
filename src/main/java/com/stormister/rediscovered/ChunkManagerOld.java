...
public class ChunkManagerOld extends WorldChunkManager {
	...
	public static int[] getBiomesGens(int par1, int par2, int par3, int par4) {
		...
		temperature = field_4194_e.func_4112_a(temperature, par1, par2, par3, par3, 0.02500000037252903D,
				0.02500000037252903D, 0.25D);
		humidity = field_4193_f.func_4112_a(humidity, par1, par2, par3, par3, 0.05000000074505806D,
				0.05000000074505806D, 0.33333333333333331D);
		field_4196_c = field_4192_g.func_4112_a(field_4196_c, par1, par2, par3, par3, 0.25D, 0.25D,
				0.58823529411764708D);
		int i1 = 0;
	...
	public void generateBiomeLookup() {
		...
		for (int i = 0; i < 64; i++) {
			for (int j = 0; j < 64; j++) {
				biomeLookupTable[i + (j * 64)] = getBetaBiomes(i / 63F, j / 63F);
			}
		}
	}
	...
	public float[] getRainfall(float[] par1ArrayOfFloat, int par2, int par3, int par4, int par5) {
		...
		for (int var7 = 0; var7 < (par4 * par5); ++var7) {
			float var8 = BiomeGenBase.getBiome(var6[var7]).getIntRainfall() / 65536.0F;
