...
public class MapGenSkyCaves extends MapGenSky {
	...
	protected void func_870_a(int i, int j, Block[] blocks, double d, double d1, double d2) {
	...
	protected void releaseEntitySkin(int i, int j, Block[] blocks, double d, double d1, double d2, float f, float f1,
		...
		for (; k < l; k++) {
			...
			if (!flag && (k == j1) && (f > 1.0F)) {
				releaseEntitySkin(i, j, blocks, d, d1, d2, (random.nextFloat() * 0.5F) + 0.5F, f1 - 1.570796F, f2 / 3F,
						k, l, 1.0D);
				releaseEntitySkin(i, j, blocks, d, d1, d2, (random.nextFloat() * 0.5F) + 0.5F, f1 + 1.570796F, f2 / 3F,
						k, l, 1.0D);
				return;
