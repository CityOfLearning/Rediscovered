...
public class BlockAbsorb extends Block {
	...
	public int makeStill(World world, int i, int j, int k) {
		...
		if ((material == Material.water)
				&& !world.getBlockState(new BlockPos(i, j, k)).getBlock().equals(Blocks.water)) {
		...
		for (int i1 = pos.getX() - byte0; i1 <= (pos.getX() + byte0); i1++) {
			for (int j1 = pos.getY() - byte0; j1 <= (pos.getY() + byte0); j1++) {
				for (int k1 = pos.getZ() - byte0; k1 <= (pos.getZ() + byte0); k1++) {
					if ((k1 > (pos.getZ() - byte0)) && (k1 < (pos.getZ() + byte0)) && (j1 > (pos.getY() - byte0))
							&& (j1 < (pos.getY() + byte0)) && (i1 > (pos.getX() - byte0)) && (i1 < (pos.getX() + byte0))
							&& flag) {
						absorbBlock(world, i1, j1, k1);
						continue;
					}

					if (flag) {
						makeStill(world, i1, j1, k1);
						continue;
					}
