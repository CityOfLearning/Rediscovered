...
public class BlockMountable extends Block {
	...
	public static boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer entityplayer,
			EnumFacing facing) {
	...
	// mounting positions.
	public static boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer entityplayer,
			EnumFacing side, float x, float y, float z, int blah) {
		if (!world.isRemote) {
			// Looks for EMBs up to 1 block away from the activated block.
			// Hopefully you didn't set the mounting position further away than
			// this.
			List<EntityMountableBlock> listEMB = world.getEntitiesWithinAABB(EntityMountableBlock.class,
					AxisAlignedBB.fromBounds(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1.0D, pos.getY() + 1.0D,
							pos.getZ() + 1.0D).expand(1D, 1D, 1D));
			for (EntityMountableBlock entitytocheck : listEMB) {
				// Looks for an EMB created by this block.
				if ((entitytocheck.getOrgBlockPosX() == pos.getX()) && (entitytocheck.getOrgBlockPosY() == pos.getY())
						&& (entitytocheck.getOrgBlockPosZ() == pos.getZ())) {
			...
			// one was bugged.
			EntityMountableBlock nemb = new EntityMountableBlock(world, entityplayer, pos.getX(), pos.getY(),
					pos.getZ(), mountingX, mountingY, mountingZ);
