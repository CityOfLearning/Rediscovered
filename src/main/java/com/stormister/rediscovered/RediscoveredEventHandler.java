...
public class RediscoveredEventHandler {
	...
	public static BlockPos verifyRespawnCoordinates(World par0World, BlockPos par1ChunkCoordinates, boolean par2,
		...
		// player.mcServer.worldServerForDimension(0).getBlockState(par1ChunkCoordinates.posX,
	...
	public void onArrowNockEvent(ArrowNockEvent event) {
		...
		if ((itemstack != null) && ((itemstack.getItem() == mod_Rediscovered.Quiver)
				|| (itemstack.getItem() == mod_Rediscovered.LeatherQuiver)
				|| (itemstack.getItem() == mod_Rediscovered.ChainQuiver)
				|| (itemstack.getItem() == mod_Rediscovered.GoldQuiver)
				|| (itemstack.getItem() == mod_Rediscovered.IronQuiver)
				|| (itemstack.getItem() == mod_Rediscovered.DiamondQuiver)
				|| (itemstack.getItem() == mod_Rediscovered.LeatherChainQuiver))) {
		...
		if (event.block.equals(mod_Rediscovered.CherrySapling)) {
			if (!world.isRemote) {
				...
				if (world.rand.nextFloat() < chance) {
					...
					((BlockCherrySapling) mod_Rediscovered.CherrySapling).generateTree(event.world, event.pos,
	...
	public void onLivingHurtEvent(LivingHurtEvent event) {
		if (!event.entityLiving.worldObj.isRemote && (event.entityLiving instanceof EntityPlayerMP)
				&& (event.entityLiving.dimension == mod_Rediscovered.DimID)
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if (!event.entityLiving.worldObj.isRemote && (event.action == Action.RIGHT_CLICK_BLOCK)
				...
				&& (((itemStack != null) && itemStack.getItem().equals(mod_Rediscovered.DreamPillow)
						&& (mod_Rediscovered.DaytimeBed || (!mod_Rediscovered.DaytimeBed && !world.isDaytime())))
						|| (player.dimension == mod_Rediscovered.DimID))
			...
			if (player.dimension == 0) {
				...
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer,
						...
						new SkyDimensionTeleporter(thePlayer.mcServer.worldServerForDimension(mod_Rediscovered.DimID)));
				...
				if (mod_Rediscovered.hasLitLanternOnHotbar(p.inventory)) {
					...
					if (mod_Rediscovered.usernameLastPosMap.containsKey(p.getDisplayNameString())) {
						...
						if (((pos2.getX() != pos.getX()) || (pos2.getY() != pos.getY()) || (pos2.getZ() != pos.getZ()))
								&& p.worldObj.getBlockState(pos2).equals(mod_Rediscovered.Lantern.getDefaultState())) {
	...
	public void onPlayerSleepInBed(PlayerSleepInBedEvent event) {
		...
		if ((player.dimension == 0) && !world.isDaytime() && (itemRand.nextInt(100) <= mod_Rediscovered.DreamChance)
			...
			props.setRespawn(event.pos.getX(), event.pos.getY(), event.pos.getZ());
			EntityPlayerMP thePlayer = (EntityPlayerMP) player;
			thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, mod_Rediscovered.DimID,
					new SkyDimensionTeleporter(thePlayer.mcServer.worldServerForDimension(mod_Rediscovered.DimID)));
		}
	}
}
