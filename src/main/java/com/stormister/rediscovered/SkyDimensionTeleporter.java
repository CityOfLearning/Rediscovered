...
import com.stormister.rediscovered.world.WorldGenSkySpawn;
...
public class SkyDimensionTeleporter extends Teleporter {
	...
	public boolean makePortal(Entity p_85188_1_) {
			...
			for (k2 = k - b0; k2 <= (k + b0); ++k2) {
				...
				for (i3 = worldServerInstance.getActualHeight() - 1; i3 >= 0; --i3) {
					if (worldServerInstance.isAirBlock(new BlockPos(i2, i3, k2))) {
						while ((i3 > 0) && worldServerInstance.isAirBlock(new BlockPos(i2, i3 - 1, k2))) {
							--i3;
						}
							...
							for (i4 = 0; i4 < 3; ++i4) {
								for (j4 = 0; j4 < 4; ++j4) {
									for (k4 = -1; k4 < 4; ++k4) {
										...
										if (((k4 < 0) && !worldServerInstance.getBlockState(new BlockPos(l4, i5, j5))
												.getBlock().getMaterial().isSolid())
												|| ((k4 >= 0)
														&& !worldServerInstance.isAirBlock(new BlockPos(l4, i5, j5)))) {
											continue label271;
										}
									}
								}
							...
							for (j3 = l1; j3 < (l1 + 2); ++j3) {
								...
								for (i4 = 0; i4 < 4; ++i4) {
									for (j4 = -1; j4 < 4; ++j4) {
										...
										if (((j4 < 0) && !worldServerInstance.getBlockState(new BlockPos(k4, l4, i5))
												.getBlock().getMaterial().isSolid())
												|| ((j4 >= 0)
														&& !worldServerInstance.isAirBlock(new BlockPos(k4, l4, i5)))) {
											continue label219;
										}
									}
								}
		...
		if (d0 < 0.0D) {
			i1 = MathHelper.clamp_int(i1, 70, worldServerInstance.getActualHeight() - 10);
			j2 = i1;
			...
			for (i3 = -1; i3 <= 1; ++i3) {
				for (j3 = 1; j3 < 3; ++j3) {
					for (k3 = -1; k3 < 3; ++k3) {
						...
						j4 = (k2 + ((j3 - 1) * l2)) - (i3 * l5);
						boolean flag = k3 < 0;
						worldServerInstance.setBlockState(new BlockPos(l3, i4, j4),
								flag ? Blocks.air.getDefaultState() : Blocks.air.getDefaultState());
					}
				}
			}
		}
		...
		for (j3 = 0; j3 < 4; ++j3) {
			...
			for (k3 = 0; k3 < 4; ++k3) {
				for (l3 = -1; l3 < 4; ++l3) {
					...
					j4 = j2 + l3;
					k4 = k2 + ((k3 - 1) * l2);
					worldServerInstance.notifyNeighborsOfStateChange(new BlockPos(i4, j4, k4),
							worldServerInstance.getBlockState(new BlockPos(i4, j4, k4)).getBlock());
				}
			}
		}
	...
	public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_) {
		...
		if (destinationCoordinateCache.containsItem(k)) {
			SkyDimensionTeleporter.PortalPosition portalposition = (SkyDimensionTeleporter.PortalPosition) destinationCoordinateCache
					.getValueByKey(k);
			d0 = 0.0D;
			...
			for (int l = -128; l <= 128; ++l) {
				...
				for (int i1 = -128; i1 <= 128; ++i1) {
					for (BlockPos blockpos = blockpos4.add(l,
						...
						if (worldServerInstance.getBlockState(blockpos).getBlock() == Blocks.bed) {
		...
		if (d0 >= 0.0D) {
			if (flag1) {
				destinationCoordinateCache.add(k, new SkyDimensionTeleporter.PortalPosition((BlockPos) object,
			...
			if (worldServerInstance.getBlockState(((BlockPos) object).west()).getBlock() == Blocks.bed) {
				enumfacing = EnumFacing.NORTH;
			}

			if (worldServerInstance.getBlockState(((BlockPos) object).east()).getBlock() == Blocks.bed) {
				enumfacing = EnumFacing.SOUTH;
			}

			if (worldServerInstance.getBlockState(((BlockPos) object).north()).getBlock() == Blocks.bed) {
				enumfacing = EnumFacing.EAST;
			}

			if (worldServerInstance.getBlockState(((BlockPos) object).south()).getBlock() == Blocks.bed) {
			...
			for (int l = -2; l <= 2; ++l) {
				for (int i1 = -2; i1 <= 2; ++i1) {
					for (int j1 = -1; j1 < 3; ++j1) {
						...
						int i2 = (k + (i1 * b1)) - (l * b0);
						boolean flag = j1 < 0;
						worldServerInstance.setBlockState(new BlockPos(k1, l1, i2),
